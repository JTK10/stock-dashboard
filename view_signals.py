...
def load_data_from_dynamodb(target_date, signal_type="ALPHA_HUNTER"):
    """
    Fetches data from DynamoDB based on Date AND Signal Type.
    """
    try:
    ...
    # FILTER BY SIGNAL TYPE (Old strategy vs New Strategy)
...
# =========================================================
# PAGE 1: EXISTING ALPHA HUNTER (Your Original Code)
# =========================================================
def render_alpha_hunter(selected_date):
    st.header("🦁 Alpha Hunter Strategy")
    
    try:
        df = load_data_from_dynamodb(selected_date, signal_type="ALPHA_HUNTER")
        
        if df.empty:
            st.warning(f"⚠️ No Alpha Hunter alerts found for {selected_date}.")
            return

        # --- Sidebar Filters ---
        st.sidebar.subheader("Filters (Alpha Hunter)")
        ...
        if 'Direction' in df.columns and selected_dirs:
        ...
        column_config = {
            ...
            "NoiseRatio": st.column_config.NumberColumn("Noise Ratio", format="%.2f", help="Lower is better"),
...
# =========================================================
# PAGE 2: NEW INTRADAY BOOST (Gainers/Losers)
# =========================================================
def render_intraday_boost(selected_date):
    st.header("🚀 Intraday Boost (Top Gainers/Losers)")
    st.info("Showing F&O stocks accumulated till 12 PM that broke PDH/PDL/PWH/PWL.")

    try:
        # Load data with Signal="INTRADAY_BOOST"
        df = load_data_from_dynamodb(selected_date, signal_type="INTRADAY_BOOST")
        
        if df.empty:
            st.warning(f"⚠️ No Intraday Boost alerts found for {selected_date}.")
            return

        # Prepare TradingView Links
        ...
        # Sort by Net Move % (Absolute value to show biggest movers top)
        if 'NetMovePct' in df.columns:
            df['AbsMove'] = df['NetMovePct'].abs()
            df = df.sort_values(by='AbsMove', ascending=False)

        # Display Columns
        display_cols = ['Chart', 'Name', 'Time', 'BreakType', 'SignalPrice', 'NetMovePct', 'RVOL']
        ...
        column_config = {
            ...
            "RVOL": st.column_config.NumberColumn("RVOL", format="%.2fx"),
            "BreakType": st.column_config.TextColumn("Broken Level", help="PDH: Prev Day High, PWH: Prev Week High")
        }
        ...
        st.error(f"Error loading Intraday Boost: {e}")
...
selected_date = st.sidebar.date_input("Select Date", today_india)

if page == "Alpha Hunter":
    render_alpha_hunter(selected_date)
